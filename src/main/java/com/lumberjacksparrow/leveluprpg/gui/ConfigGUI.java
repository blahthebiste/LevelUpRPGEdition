...
public final class ConfigGUI extends GuiScreen implements IModGuiFactory {
    ...
    protected void actionPerformed(GuiButton button) {
        if (button.enabled) {
            if (button.id == 0) {
                ...
                button.displayString = I18n.format("config.leveluprpg.option" + (button.id - 1), toggles[button.id - 1]);
            }
        }
    }
    ...
    public void drawScreen(int par1, int par2, float par3) {
        ...
        this.drawCenteredString(this.fontRenderer, I18n.format("config.leveluprpg.title"), this.width / 2, this.height / 2 - 115, 0xFFFFFF);
    ...
    public void onGuiClosed() {
        super.onGuiClosed();
        LevelUpRPG.instance.refreshValues(toggles);
    }
