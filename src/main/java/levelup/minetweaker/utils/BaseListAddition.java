...
public abstract class BaseListAddition<T> extends BaseListModification<T> {
    ...
    public String describe() {
        return String.format("[LevelUp!] Adding %d %s blacklist(s) for %s", recipes.size(), name, getRecipeInfo());
    }

    @Override
    public String describeUndo() {
        return String.format("[LevelUp!] Removing %d %s blacklist(s) for %s", recipes.size(), name, getRecipeInfo());
    }
}